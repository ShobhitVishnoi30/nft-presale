...
contract NFTPresale is ERC721Enumerable, Ownable, Pausable {
    ...
    function mint(uint256 _mintAmount) external payable whenNotPaused {
        ...
        require(
            supply + _mintAmount <= maximumNFTSupply,
        ...
        if (block.timestamp <= presaleOpenTime + presaleDuration) {
            ...
            require(
                msg.value >= _presaleCost * _mintAmount,
                "insufficient funds"
            );
        } else if (block.timestamp > presaleOpenTime + presaleDuration) {
            require(
                msg.value >= _normalCost * _mintAmount,
                "insufficient funds"
            );
        }
    ...
    function getPrice() external view returns (uint256) {
        if (_deploymentTime <= presaleOpenTime + presaleDuration) {
            return _normalCost;
        } else {
            return _presaleCost;
        }
